...
export class ProjectPhaseService {
    ...
    async findOneById(id: string): Promise<ProjectPhase> {
        const phase = await this.phaseRepository.findOne({
            where: { id },
            relations: ['project', 'phaseStatus', 'phaseName', 'documents', 'projectPhases'],
        });
    ...
    async create(createDto: CreateProjectPhaseDto): Promise<ProjectPhase|ProjectPhase[]> {
        ...
        let phaseName = createDto.phaseName?.name
            ? await this.phaseNameRepository.findOneBy({ name: createDto.phaseName.name })
            : undefined;
        if (!phaseName) {
        ...
        let phaseStatus = createDto.status?.name
            ? await this.phaseStatusRepository.findOneBy({ name: createDto.status.name })
            : undefined;

        if (createDto.status?.name && !phaseStatus) {
            throw new BadRequestException(`Phasenstatus mit ID ${createDto.status.id} nicht gefunden.`);
        }
    ...
    async update(id: string, updateDto: UpdateProjectPhaseDto): Promise<ProjectPhase> {
        const phase = await this.findOneById(id);

        // Falls `statusId` übergeben wird, sicherstellen, dass der Status existiert
        if (updateDto.status?.name) {
            const phaseStatus = await this.phaseStatusRepository.findOneBy({ name: updateDto.status.name });
            if (!phaseStatus) {
                throw new BadRequestException(`Phasenstatus mit Name ${updateDto.status.name} nicht gefunden.`);
            }
            phase.phaseStatus = phaseStatus;
        }

        // Falls `projectId` übergeben wird, sicherstellen, dass das Projekt existiert
        if (updateDto.projectId) {
            const project = await this.projectRepository.findOneBy({ id: updateDto.projectId });
            if (!project) {
        ...
        if (updateDto.phaseName !== undefined) phase.phaseName.name = updateDto.phaseName.name;
